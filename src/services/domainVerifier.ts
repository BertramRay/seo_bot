 ...
 * 域名验证服务
 */
export default class DomainVerifier {
  /**
   * 安排域名验证
   * @param userId - 用户ID
   * @param domain - 要验证的域名
   * @param isCustomDomain - 是否为自定义域名
   * @returns 更新后的用户
   */
  static async scheduleVerification(userId: string, domain: string, isCustomDomain = false): Promise<IUser> {
    try {
      ...
      user.domainVerifiedAt = undefined;
      await user.save();
   ...
   * @returns 验证是否成功
   */
  static async verifyDomain(userId: string, isCustomDomain = false): Promise<boolean> {
    try {
      const user = await User.findById(userId);
      if (!user) {
        throw new Error('用户不存在');
   ...
   * @returns 验证状态对象
   */
  static async getDomainStatus(userId: string, isCustomDomain = false): Promise<DomainStatus> {
    try {
      const user = await User.findById(userId);
      if (!user) {
        throw new Error('用户不存在');
   ...
   * @returns DNS配置指南
   */
  static generateDnsGuide(domain: string, isCustomDomain = false): DnsGuide {
    // 本地测试模式
    if (config.server.env === 'development' && (config.domain.baseDomain === 'localhost' || config.domain.dnsProvider === 'local')) {
      if (isCustomDomain) {
        return {
